...
def dump(trace, fpath):
    ...
    with open(fpath, 'w') as fo:
        for packet in trace:
            fo.write(str(packet) + ct.NL)  # str()：将数值转化为字符串

class Packet(object):
    """Define a packet.
    Direction is defined in the rbp constant.py.
    """
    payload = None

    def __init__(self, timestamp, direction, length, dummy=False):  # 设置数据包Packet类的属性，有时间戳、方向、数据包长度和是否为虚拟数据包
        self.timestamp = timestamp
        self.direction = direction
        self.length = length
        self.dummy = dummy

    def __lt__(self, other):  # lt=less than，即小于
